...
IMAGE_DIR="${IMAGE_DIR:-$PROJECT_ROOT/tests/detect-images}"
REQUESTS=200
CONCURRENCY=4
WARMUP_REQUESTS=1
...
Options:
  -n  Total number of benchmark requests per run. Default: 20
  -c  Parallel requests to issue during the concurrent run. Default: 4
...
Environment:
  ...
  MODEL_PATH Path to the YOLO ONNX model. Default: ../models/yolov8n.onnx
EOF
}
...
SERVER_LOG=$(mktemp -t cvserve-python-benchmark-server.XXXXXX.log)
SEQ_RESULTS_FILE=$(mktemp -t cvserve-python-benchmark-seq.XXXXXX.txt)
CONC_RESULTS_FILE=$(mktemp -t cvserve-python-benchmark-conc.XXXXXX.txt)
WARMUP_FILE=$(mktemp -t cvserve-python-benchmark-warmup.XXXXXX.txt)
RESPONSE_DIR=$(mktemp -d -t cvserve-python-benchmark-responses.XXXXXX)
...
MODEL_PATH="$MODEL_PATH" python3 -m uvicorn app.main:app --host 127.0.0.1 --port 9000 >"$SERVER_LOG" 2>&1 &
SERVER_PID=$!
